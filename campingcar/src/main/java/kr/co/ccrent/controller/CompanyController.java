...
//	신청하기 작성
	...
	public String register(CompanyDTO companyDTO, HttpServletRequest request, RedirectAttributes rttr)
			throws Exception {
...
//	신청상태 변경하기 comp_status +1 = 승인
	@RequestMapping(value = "/admin/company/modify", method = RequestMethod.POST)
	public String modify(Criteria cri, Model model,@RequestParam("comp_id") int comp_id) throws Exception {
		CompanyDTO companyDTO = companyService.get(comp_id);
		model.addAttribute("list2", companyService.getAll2(cri));
		model.addAttribute("list3", companyService.getAll3(cri));
...
//  신청 상태 거절하기 ( 삭제 )
	@RequestMapping(value = "/admin/company/remove", method = RequestMethod.GET)
	public String modify2(Criteria cri, Model model,@RequestParam("comp_id") int comp_id) throws Exception {

		model.addAttribute("list2", companyService.getAll2(cri));
		model.addAttribute("list3", companyService.getAll3(cri));
		companyService.remove(comp_id);
		// 페이징처리
		PageMaker pageMaker = new PageMaker();
		pageMaker.setCri(cri);
		pageMaker.setTotalCount(companyService.getAll2_Count(cri));
